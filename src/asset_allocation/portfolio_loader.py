...
def create_asset_class_or_category(data: dict, holdings: Dict[str, int], quote_service: QuoteService) -> Union[AssetClass, AssetClassCategory]:
...
def load_portfolio(hierarchy_file: str, holdings_file: str, quote_service: QuoteService) -> Portfolio:
    ...
    cash_target = hierarchy_data.get('cash_target', 0.0)  # Default to 0.0 if not specified
    
    # Create the asset class hierarchy
    children = [
        create_asset_class_or_category(investment, holdings_data['holdings'], quote_service)
